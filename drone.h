...
//Display
// Add these to your public section if they aren't already there:
void sortByAscending();    // alphabetical A->Z on tasks[], keep positions paired
void sortByDescending();   // Z->A on tasks[], keep positions paired
void operator<<(const drone& d2);  // copy ALL attributes from d2 into *this
void displayDrone() const;         // pretty print (used when retrieving or debugging)
